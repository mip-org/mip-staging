 ...
 * own their data.  fmm2d.c's mexFunction is invoked through mex_dispatch();
 * errors raised via mexErrMsgTxt longjmp out to the dispatch entry, which
 * then surfaces the message to JS.
 */
...
int mexEvalString(const char* /*str*/) {
    /* No-op outside MATLAB.  Not used by fmm2d.c but kept for header
     * completeness. */
    return 0;
}
...
/* fmm2d.c is plain C, so its mexFunction has C linkage. */
