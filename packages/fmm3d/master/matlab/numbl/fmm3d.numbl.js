...
// wasm: fmm3d
//
// Single-builtin replacement for the upstream fmm3d MEX file.
//
// The upstream `matlab/lfmm3d.m`, `hfmm3d.m`, `stfmm3d.m`, `emfmm3d.m` and
// their direct-sum references all dispatch through `fmm3d(mex_id_, ...args)` where
...
// fmm3d.c), and decodes the resulting plhs[] back into runtime values.
//
// fmm3d's stubs are stateless (no plan/setpts/destroy lifecycle like
...
function buildMxArray(bridge, v) {
  ...
  if (isClassInstance(v)) {
    ...
    if (mw === undefined) {
      throw new RuntimeError("fmm3d: class instance has no mwptr field");
    }
    return buildMxArray(bridge, mw);
  }
  throw new RuntimeError("fmm3d: unsupported argument type " + (v && v.kind));
}
...
function decodeMxArray(bridge, mx) {
  ...
  throw new RuntimeError("fmm3d: unsupported output mxArray classID " + classID);
}
...
function callFmm3d(args, nargout) {
  ...
  // outputs (no nlhs check).  rfmm3d_ndiv writes 8 plhs slots, so we
  ...
  try {
    ...
    if (rc !== 0) {
      throw new RuntimeError("fmm3d: " + bridge.getError());
    }
