...
// wasm: fmm3d_legacy
//
// Single-builtin replacement for the upstream fmm3d_legacy MEX file.
//
// The upstream `matlab/lfmm3d_legacy.m`, `hfmm3d_legacy.m`, `stfmm3d_legacy.m`, `emfmm3d_legacy.m` and
// their direct-sum references all dispatch through `fmm3d_legacy(mex_id_, ...args)` where
...
// fmm3d_legacy.c), and decodes the resulting plhs[] back into runtime values.
//
// fmm3d_legacy's stubs are stateless (no plan/setpts/destroy lifecycle like
  ...
  resolve: function (argTypes, nargout) {
    ...
    return {
      ...
      apply: function (args, nargout) {
        return callFmm3dLegacy(args, nargout);
      },
    };
  },
});
...
function buildMxArray(bridge, v) {
  ...
  if (isClassInstance(v)) {
    ...
    if (mw === undefined) {
      throw new RuntimeError("fmm3d_legacy: class instance has no mwptr field");
    }
    return buildMxArray(bridge, mw);
  }
  throw new RuntimeError("fmm3d_legacy: unsupported argument type " + (v && v.kind));
}
...
function decodeMxArray(bridge, mx) {
  ...
  throw new RuntimeError("fmm3d_legacy: unsupported output mxArray classID " + classID);
}
...
function callFmm3dLegacy(args, nargout) {
  ...
  // outputs (no nlhs check).  rfmm3d_legacy_ndiv writes 8 plhs slots, so we
  // need at least that many.  Allocate a generous fixed cap.
  var plhsSlots = nlhs > 32 ? nlhs : 32;
  ...
  try {
    ...
    if (rc !== 0) {
      throw new RuntimeError("fmm3d_legacy: " + bridge.getError());
    }
